...
def fitness_func(solution: np.ndarray, sol_idx: int):
    global data_inputs, data_outputs, keras_ga, model

    predictions = pygad.kerasga.predict(
        model=model, solution=solution, data=data_inputs
    )
...
data_inputs = np.array(
    [[0.02, 0.1, 0.15], [0.7, 0.6, 0.8], [1.5, 1.2, 1.7], [3.2, 2.9, 3.1]]
)

# Data outputs
data_outputs = np.array([[0.1], [0.6], [1.3], [2.5]])
